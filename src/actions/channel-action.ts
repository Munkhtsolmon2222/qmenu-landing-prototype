...
export async function GET_ES_CHANNELS(input: EsQueryInput) {
  return query<EsQueryResult>({ query: CHANNEL_QUERY.GET_ES_CHANNELS, variables: { input } });
}
...
export async function GET_ES_CHANNELS_BY_PRODUCTS(input: EsQueryInput) {
  return query<EsQueryResult>({
    query: CHANNEL_QUERY.GET_ES_CHANNELS_BY_PRODUCTS,
    variables: { input },
  });
}
