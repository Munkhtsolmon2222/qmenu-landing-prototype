...
const GalleryTab = ({ branch }: Props) => {
  ...
  const images = useMemo(() => {
    ...
    let count = 1;
    const result = imgs.reduce((res: any[], img, i) => {
...
const ImgItem: React.FC<{ img: string; className?: string }> = ({ img, className }) => {
  return (
    <div className={cn('rounded-lg overflow-hidden min-h-36 max-h-48', className)}>
