...
const Event: React.FC<Props> = ({ participant }) => {
  ...
  const types: EventType[] = Array.from(new Set(events.map((event) => event.type))).sort((a, b) =>
    typeName[b].localeCompare(typeName[a]),
  );
      ...
      >
        ...
        <div className="w-full xl:w-4/5 grid grid-cols-1 md:grid-cols-2 gap-5">
          {events
            ...
            .map((event, index) => (
              <EventCard event={event} key={index} onClick={() => onSelect(event)} />
            ))}
        </div>
      </div>
      <br />
    ...
    <nav className="w-1/4 xl:w-1/5 sticky z-40 sm:top-20 top-16 self-start bg-background col-span-2">
      <div className="overflow-y-auto px-0 sm:px-3 no-scrollbar gap-2 flex flex-col">
        ...
        `}
              onClick={() => setType(e)}
            >
              {active && <div className="h-full absolute top-0 left-0 w-1 sm:w-1.5 bg-current-2" />}
              <span className="ml-1 sm:ml-3 text-sm sm:text-base truncate">{typeName[e]}</span>
            </div>
          );
        })}
      </div>
