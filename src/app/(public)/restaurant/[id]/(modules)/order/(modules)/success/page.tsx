...
import { PAGE_ORDERS, PAGE_RESTAURANT } from "@/lib/config/page";
...
const Index: React.FC = () => {
  ...
  const { data: { getOrder: order } = { getOrder: null }, loading } = useQuery<{
    getOrder: Order;
  }>(GET_ORDER, {
    skip: !searchParams.get("orderId"),
    variables: { id: searchParams.get("orderId") },
  });

  const handleClick = () => {
    router.push(`${PAGE_RESTAURANT}/${id}`);
    setTimeout(() => router.push(PAGE_ORDERS + `?key=1`), 100);
  };
  ...
  return (
    <>
        ...
        <div className="flex flex-col items-center">
          ...
          {order?.number && (
            <div className="text-center mt-5">
              <span className="text-secondary-text opacity-75 mr-2">
      ...
      <OrderDialog.Footer
        onClick={() => router.push(`${PAGE_RESTAURANT}/${id}`)}
      >
