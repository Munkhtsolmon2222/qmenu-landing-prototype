...
function ListFilter(props: Props) {
  ...
  const { data: categories } = useQuery(GET_MASTER_CATEGORIES);
  ...
  const array = [
    {
      key: "cuisine",
      name: t("Food type"),
      filters: tags?.map((e) => <Translate key="e">{e.name}</Translate>),
    },
    {
      key: "tags",
      name: t("Advantage"),
      filters: filter?.map((e) => <Translate key="e">{e.name}</Translate>),
    },
    {
      key: "price",
      name: t("Price"),
    ...
    },
    {
      key: "recommended",
      name: t("Order type"),
      filters: ORDERS_ARRAY.map((e) => (
        <Translate key="e">{e.label}</Translate>
      )),
    },
    {
      key: "category",
      name: t("Category"),
      filters: categories?.getMasterCategories?.map((e) => (
        <Translate key="e">{e.name}</Translate>
      )),
    },
    {
      key: "reviews",
