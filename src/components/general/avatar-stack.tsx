...
const avatarStackVariants = cva('flex', {
  variants: {
    variant: {
      default: 'gap-1',
      stack: 'hover:space-x-1.5 -space-x-2.5 rtl:space-x-reverse cursor-pointer',
    },
  },
  defaultVariants: {
    variant: 'default',
          ...
          <Tooltip open={avatar.name ? undefined : false}>
            <TooltipTrigger asChild>
              <Avatar size={size} className={cn(avatar.wrapperClassName)}>
                ...
                <AvatarFallback className={cn(colors[index], 'text-white', avatar.className)}>
                  {avatar.children ??
                    avatar.name.split(' ').reduce((acc, subName) => acc + subName[0], '')}
                </AvatarFallback>
              </Avatar>
            </TooltipTrigger>
            <TooltipContent>{avatar.tooltipChildren ?? <p>{avatar.name}</p>}</TooltipContent>
          </Tooltip>
        </TooltipProvider>
      ))}
