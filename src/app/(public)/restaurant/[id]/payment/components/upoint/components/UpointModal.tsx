import { Icons, DialogFooter } from '@/components/general';
...
export const UpointModal = ({ visible, onClose, onSubmit }: Props) => {
    ...
    <div className="p-2 px-8" key={2}>
      ...
      <div className="flex items-center border-b border-border py-2">
        ...
        <div className="flex flex-col whitespace-nowrap justify-center items-end ml-auto">
          ...
          <span className="text-base text-primary">{upointBalance?.balance}</span>
        </div>
      </div>
  ...
  const renderFooter = [
    <DialogFooter key={1} className="flex justify-center sm:justify-center py-2 relative">
        ...
        <div
          ...
          className={`flex justify-center items-center p-2 h-9 rounded-lg border  ${
            upointBalance?.state === 'spend'
              ? 'bg-lightapricot text-current-2 border-current-2'
  ...
  return (
    <Dialog open={visible} onOpenChange={onClose}>
      <DialogContent className="w-[380px] rounded-lg" onClick={() => ref.current?.focus()}>
