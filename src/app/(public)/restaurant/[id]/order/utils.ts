import { SEAT_DURATION } from '@/lib/constant';
...
export const getDates = (timetable?: Timetable) => {
  ...
  const lastDate = dates.findIndex((e) => filteredDates[filteredDates.length - 1].key === e.key);
...
const getTimetable = (dateValue: Date, timetable?: Timetable) => {
  ...
  let openTime = (timetable?.[obj[1] as keyof Timetable] ?? '00:00') as string;
  ...
  if (isOpen) {
    for (let hour = 0; hour < 24; hour++) {
      for (const minute of timesArray) {
        ...
        if (checkTime(openTime, closeTime, getDateByTime(time))) times.push(time);
      }
    }
  }
